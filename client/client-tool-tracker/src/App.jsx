...
function App() {
  ...
  const exportToExcel = () => {
    const data = {name : assets.name ,cdsid : assets.cdsid , location : assets.location ,asset_category : assets.asset_category , asset_id : assets.asset_id , project : assets.project} 
  ...
  const fetchAssets = async (filterType , filterValue) => {
    if (filterValue !== "" && filterType !== "NONE") {
      try {
        ...
        const response = await fetch(`/assets/find-by-category/${filterType}/${filterValue}`);
